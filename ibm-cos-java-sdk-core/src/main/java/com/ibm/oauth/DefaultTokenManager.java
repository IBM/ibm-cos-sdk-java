...
/**
 * Default implementation of Token Manager. This design will
 * allow the class to be created with multiple constructors, yet only one instance 
 * can be created. The DefaultTokenProvider is used to retrieve the token from the IAM Service
 * which uses the api key method. A client can also use their own implementation of TokenHandler
 * 
 *
 */
public class DefaultTokenManager implements TokenManager{
	...
	/**
	 * Default constructor using the apiKey. This instance will use the DefaultTokenProvider to 
	 ...
	 * 
	 * @param provider
	 * 			The Token Provider which will retrieve the Token Object from the IAM service
	 */
	...
	public String getToken() {
		...
		if(isTokenExpiring(token)) {
			retrieveIAMTokenAsync(token.getRefresh_token());
		}
	 ...
	 * 
	 * @return boolean
	 * 			Indicates if the Token has been cached
	 */
	...
	/**
	 * Check if the current cached token has expired. If it has a synchronous http call
	 * is made to the IAM service to retrieve & store a new token
	 * 
	 * @param token
	 * 			The IAM Token object
	 * 
	 * @return boolean
	 * 			Indicates if the currently cached token has expired
	 */
	...
	/**
	 * Check if the current cached token is expiring in less than 15 seconds. 
	 * If it is, an asynchronous call is made to the IAM service to update the cache.
	 * 
	 * @param token
	 * 			The IAM Token object
	 * 
	 * @return boolean
	 * 			Indicates if the currently cached token is about to expire in 15 seconds
	 */
	...
	/**
	 * Retrieve the Token from IAM using a HttpClient asynchronously. 
	 * The token will replace the currently cached token
	 * 
	 * @param refreshToken
	 */
	...
	/**
	 * retrieve token from provider. Ensures each thread checks the token is null
	 * prior to making the callout to IAM
	 * 
	 */
